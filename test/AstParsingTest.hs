...
testParseLambda =
  TestCase
    ( assertEqual
        "should parse lambda with args and body"
        expected
        (parseAstFromSExpr sexpr)
    )
  where
...
testParseLambdaArgsMalformed =
  TestCase
    ( assertBool
        "lambda should fail with non-symbol args"
...
testParseIfMalformed =
  TestCase
    ( assertBool
        "if should be malformed when not 3 branches"
        ( case parseAstFromSExpr sexpr of
            Left ("if", _, _) -> True
            _ -> False
        )
