...
testPrimAddSuccess = TestCase (assertEqual "primAdd 2 + 3 = 5"
    (Right (VInt 5))
    (primAdd [VInt 2, VInt 3]))

testPrimAddFail :: Test
testPrimAddFail = TestCase (assertEqual "primAdd with wrong args should fail with correct message"
    (Left "primAdd: expected two integers, got [VInt 2,VString \"ZIZI\"]")
    (primAdd [VInt 2, VString "ZIZI"]))

testPrimSubSuccess :: Test
testPrimSubSuccess = TestCase (assertEqual "primSub 5 - 3 = 2"
    (Right (VInt 2))
    (primSub [VInt 5, VInt 3]))

testPrimSubFail :: Test
testPrimSubFail = TestCase (assertEqual "primSub with wrong args should fail with correct message"
    (Left "primSub: expected two integers, got [VInt 5,VBool True]")
    (primSub [VInt 5, VBool True]))

testPrimMulSuccess :: Test
testPrimMulSuccess = TestCase (assertEqual "primMul 4 * 3 = 12"
    (Right (VInt 12))
    (primMul [VInt 4, VInt 3]))

testPrimMulFail :: Test
testPrimMulFail = TestCase (assertEqual "primMul with wrong args should fail with correct message"
    (Left "primMul: expected two integers, got [VInt 4,VBool True]")
    (primMul [VInt 4, VBool True]))

testPrimDivSuccess :: Test
testPrimDivSuccess = TestCase (assertEqual "primDiv 10 / 2 = 5"
    (Right (VInt 5))
    (primDiv [VInt 10, VInt 2]))

testPrimDivFail :: Test
testPrimDivFail = TestCase (assertEqual "primDiv with wrong args should fail with correct message"
    (Left "primDiv: expected two integers, got [VInt 10,VBool True]")
    (primDiv [VInt 10, VBool True]))

testPrimDivByZero :: Test
testPrimDivByZero = TestCase (assertEqual "primDiv by zero should fail with correct message"
    (Left "primDiv: division by zero")
    (primDiv [VInt 10, VInt 0]))

testPrimEqSuccess :: Test
testPrimEqSuccess = TestCase (assertEqual "primEq 3 == 3 = True"
    (Right (VBool True))
    (primEq [VInt 3, VInt 3]))

testPrimEqFail :: Test
testPrimEqFail = TestCase (assertEqual "primEq with wrong args should fail with correct message"
    (Left "primEq: expected two values, got [VInt 3]")
    (primEq [VInt 3]))

testPrimLtSuccess :: Test
testPrimLtSuccess = TestCase (assertEqual "primLt 2 < 3 = True"
    (Right (VBool True))
    (primLt [VInt 2, VInt 3]))

testPrimLtFail :: Test
testPrimLtFail = TestCase (assertEqual "primLt with wrong args should fail with correct message"
    (Left "primLt: expected two integers, got [VInt 2,VBool True]")
    (primLt [VInt 2, VBool True]))

testPrimGtSuccess :: Test
testPrimGtSuccess = TestCase (assertEqual "primGt 5 > 3 = True"
    (Right (VBool True))
    (primGt [VInt 5, VInt 3]))

testPrimGtFail :: Test
testPrimGtFail = TestCase (assertEqual "primGt with wrong args should fail with correct message"
    (Left "primGt: expected two integers, got [VInt 5,VBool True]")
    (primGt [VInt 5, VBool True]))

testPrimModSuccess :: Test
testPrimModSuccess = TestCase (assertEqual "primMod 10 mod 3 = 1"
    (Right (VInt 1))
    (primMod [VInt 10, VInt 3]))

testPrimModFail :: Test
testPrimModFail = TestCase (assertEqual "primMod with wrong args should fail with correct message"
    (Left "primMod: expected two integers, got [VInt 10,VBool True]")
    (primMod [VInt 10, VBool True]))

testPrimModByZero :: Test
testPrimModByZero = TestCase (assertEqual "primMod by zero should fail with correct message"
